var CustomShaders = function(){
	...
	this.gridShader = {
		...
		fragmentShader: [
			...
			"vec4 color = mix(vec4(0.5+0.5*cos(0.0), 1.0 - uv.yx,1.0), vec4(uv.yx,0.5+0.5*sin(0.0),1.0), (0.5 + 0.5*sin((-1.0 + uv.x*2.0)*(-1.0 + uv.y*2.0)*5.0)));",
	...
	this.flowShader = {
		...
		fragmentShader: [
				...
				"    for( int i=0; i<20; i++ ){",
				"    	float h  = dot( texture2D(texture, uv*0.99         ).xyz, vec3(1.0) );",
